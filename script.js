/* ================================================================
   ...
   GSAP 3 (solo plugins GRATUITOS: CustomEase)
   Sin MorphSVGPlugin — morph simulado con crossfade + scale
================================================================ */
...
/* ── 3. INICIALIZAR STROKE-DASHOFFSET ──
   ...
   Oculta todos los paths para animarlos después.         */
function initTreePaths() {
  ...
  paths.forEach(el => {
    ...
    } catch (e) { /* elementos sin getTotalLength (ellipse) se ignoran */ }
  });
  // Nudos: ocultar por opacidad
  gsap.set('.knot', { opacity: 0 });
}

/* ── 4. GENERADOR DE 200 PÉTALOS (corazón paramétrico) ── */
function generateLeaves() {
  const svgNS = 'http://www.w3.org/2000/svg';
  const SW = window.innerWidth;
  const SH = window.innerHeight;

  // Centro del follaje: parte superior del árbol
  const cx = SW * 0.50;
  const cy = SH * 0.30;
  ...
  // Capas de densidad: borde / zona media / núcleo
  const layers = [
  ...
  layers.forEach(layer => {
    for (let i = 0; i < layer.count; i++) {
      // Distribución uniforme con offset áureo para evitar agrupaciones
      const t = (i / layer.count) * Math.PI * 2 + idx * 0.618033;

      // Fórmula paramétrica del corazón (normalizada)
      ...
      // Tamaño: hojas del centro ligeramente más grandes
      ...
      // Forma de pétalo ovalado con punta inferior
  ...
  // Ordenar de adentro → afuera para stagger centrifugo
...
function dropLeaves() {
  ...
  // Mezclar Fisher-Yates y tomar 20
...
function runMasterTimeline() {
  ...
  ──────────────────────────────────────────────── */
    ...
    // Caída libre (morphWrap contiene ya el círculo visible)
    .to(morphWrap, {
      y:       '55vh',
      scaleY:  1.0,
      scaleX:  1.0,
    ...
    // SQUASH al impacto — se aplana bruscamente
    .to(morphWrap, {
      scaleY:          0.38,
      scaleX:          1.60,
  ...
  ──────────────────────────────────────────────── */
    ...
    .call(() => {
      typewriterEffect(() => {
        // Al terminar el texto → firma + contador
